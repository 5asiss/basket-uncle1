...
# 1. 초기 설정
app = Flask(__name__)
app.secret_key = "basket_uncle_direct_trade_key_999_secure" # 보안 강화를 위해 키 변경
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///direct_trade_mall.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# 테스트용 API 키 (실 서비스 시 라이브 키로 변경 필요)
...
class Category(db.Model):
    ...
    tax_type = db.Column(db.String(20), default='과세') # 카테고리별 세금 설정
    manager_email = db.Column(db.String(120), nullable=True) # 카테고리 담당 관리자
...
class Cart(db.Model):
    ...
    tax_type = db.Column(db.String(20), default='과세') # 결제 시 계산용
...
class Order(db.Model):
    ...
    tax_free_amount = db.Column(db.Integer, default=0) # 면세 합계 저장
...
def check_admin_permission(category_name=None):
    ...
    if current_user.is_admin: return True # 마스터 관리자
    # 특정 카테고리 담당자인지 확인
    if category_name:
...
<head>
    ...
    <title>바구니삼촌몰 - 카테고리별 전문 대행</title>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <section class="mb-12">
            <div class="mb-6 border-b border-gray-100 pb-4">
                ...
                <p class="text-[11px] md:text-xs text-orange-500 font-bold mt-1 bg-orange-50 px-3 py-1 rounded-full w-fit">
                    ⏰ 오후 8시 주문마감 / 🚚 다음날 오후 5시 이전 배송 / <span class="text-green-600">{{ cat.tax_type }} 적용</span>
                </p>
            </div>
                ...
                <div class="product-card bg-white rounded-2xl md:rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden relative flex flex-col w-[180px] md:w-[240px] transition-all hover:shadow-md {% if is_expired or p.stock <= 0 %}sold-out{% endif %}">
                    ...
                    <a href="/product/{{p.id}}" class="relative aspect-square block bg-white overflow-hidden">
                        ...
                        <div class="absolute bottom-2 left-2 flex flex-col gap-1">
                            <span class="bg-black/70 text-white text-[9px] px-2 py-1 rounded-md font-bold backdrop-blur-sm">
                                잔여: {{ p.stock }}개
                            </span>
                        </div>
...
def product_detail(pid):
    ...
    <div class="max-w-4xl mx-auto px-4 py-8 md:py-12">
        <div class="grid md:grid-cols-2 gap-8 md:gap-12 mb-16">
            ...
            <div class="flex flex-col justify-center">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span class="bg-gray-100 text-gray-500 px-2 py-1 rounded text-[9px] font-bold">{{ p.tax_type }}</span>
                </div>
                ...
                <div class="space-y-1 mb-8 text-xs md:text-sm text-gray-400 font-bold">
                    ...
                    <p class="text-blue-500 font-black"><i class="fas fa-warehouse mr-2"></i> 현재 잔여수량: {{ p.stock }}개</p>
                </div>
                ...
                <button onclick="addToCart('{{p.id}}')" class="w-full bg-green-600 text-white py-5 rounded-2xl font-black text-lg md:text-xl shadow-xl hover:bg-green-700 transition active:scale-95">장바구니 담기</button>
                {% else %}
        ...
        <div class="border-t pt-10">
            <h3 class="font-black text-xl md:text-2xl mb-8 border-l-4 border-green-600 pl-4">삼촌의 대행 현장 상세 정보</h3>
...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            flash("이미 가입된 이메일입니다."); return redirect('/register')
...
def add_cart(pid):
    ...
    if item: item.quantity += 1
        ...
        # 상품 등록 당시의 세금 정보 반영
        db.session.add(Cart(user_id=current_user.id, product_id=pid, product_name=p.name, price=p.price, tax_type=p.tax_type))
    db.session.commit()
    cart_count = Cart.query.filter_by(user_id=current_user.id).count()
    ...
    <script>
        ...
        document.getElementById('payment-button').addEventListener('click', function() {{
            tossPayments.requestPayment('카드', {{
                amount: {total},
                taxFreeAmount: {tax_free_amount}, // 면세 금액 전달
                orderId: '{order_id}',
...
def payment_success():
    ...
    if res.status_code == 200:
        ...
        db.session.add(Order(user_id=current_user.id, customer_name=current_user.name, customer_phone=current_user.phone, customer_email=current_user.email, product_details=details, total_price=int(amt), tax_free_amount=tax_free_total, order_id=oid, payment_key=pk, delivery_address=addr, request_memo=current_user.request_memo))
        for i in items:
...
def admin_dashboard():
    ...
    <div class="max-w-7xl mx-auto py-10 px-4">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-xl font-black text-orange-700 italic uppercase">Admin Dashboard {% if not current_user.is_admin %}(Manager){% endif %}</h2>
            <p class="text-[10px] text-gray-400 font-bold">{{ current_user.email }}</p>
        </div>
        ...
        {% if tab == 'products' %}
            <div class="flex justify-between items-center mb-6">
                <form action="/admin" class="flex gap-2"><input type="hidden" name="tab" value="products"><select name="category" onchange="this.form.submit()" class="border p-2 rounded-xl text-xs font-black bg-white"><option value="전체">전체보기</option>{% for c in categories %}{% if current_user.is_admin or c.name in my_categories %}<option value="{{c.name}}" {% if sel_cat == c.name %}selected{% endif %}>{{c.name}}</option>{% endif %}{% endfor %}</select></form>
            ...
            <div class="bg-white rounded-2xl shadow-sm border overflow-hidden text-[10px]">
                <table class="w-full text-left">
                    <thead class="bg-gray-50 border-b"><tr><th class="p-4">상품명/세금</th><th class="p-4 text-center">재고</th><th class="p-4">가격</th><th class="p-4 text-center">관리</th></tr></thead>
        ...
        {% elif tab == 'orders' %}
            ...
            <div class="bg-white rounded-2xl shadow-sm border overflow-hidden text-[9px]">
                <table class="w-full text-left">
                    <thead class="bg-gray-50 border-b"><tr><th class="p-4">일시/고객</th><th class="p-4">주소/요청사항</th><th class="p-4 text-right">면세/총금액</th></tr></thead>
                    <tbody>{% for o in orders %}<tr class="border-b"><td class="p-4 leading-relaxed font-bold">{{ o.created_at.strftime('%m/%d %H:%M') }}<br>{{ o.customer_name }}</td><td class="p-4 leading-relaxed"><span class="text-blue-600">{{ o.delivery_address }}</span><br><span class="text-orange-500">📝{{ o.request_memo }}</span></td><td class="p-4 text-right font-black"><span class="text-gray-300 text-[8px]">(면세:{{ "{:,}".format(o.tax_free_amount) }})</span><br>{{ "{:,}".format(o.total_price) }}원</td></tr>{% endfor %}</tbody>
                </table>
            </div>
        ...
        {% elif tab == 'categories' and current_user.is_admin %}
            <div class="max-w-2xl space-y-4 text-xs font-black">
                <form action="/admin/category/add" method="POST" class="bg-white p-6 rounded-2xl border flex flex-col gap-3">
                    <p class="text-gray-400">새 카테고리 & 권한 추가</p>
                    <div class="flex gap-2">
                        <input name="cat_name" placeholder="이름 (예: 정육)" class="border p-3 rounded-xl flex-1" required>
                        <select name="tax_type" class="border p-3 rounded-xl"><option value="과세">기본 과세</option><option value="면세">기본 면세</option></select>
                    </div>
                    <input name="manager_email" placeholder="담당 관리자 이메일 (비워두면 마스터 전용)" class="border p-3 rounded-xl w-full">
                    <button class="bg-green-600 text-white py-3 rounded-xl">카테고리 생성</button>
                </form>
                <div class="bg-white rounded-2xl border overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-gray-50 border-b"><tr><th class="p-4">카테고리명</th><th class="p-4">세금</th><th class="p-4">담당자</th><th class="p-4">삭제</th></tr></thead>
                        <tbody>
                        {% for c in categories %}
                        <tr class="border-b">
                            <td class="p-4">{{ c.name }}</td>
                            <td class="p-4"><span class="{% if c.tax_type == '면세' %}text-blue-500{% else %}text-gray-400{% endif %}">{{ c.tax_type }}</span></td>
                            <td class="p-4 text-gray-500">{{ c.manager_email or '-' }}</td>
                            <td class="p-4"><a href="/admin/category/delete/{{c.id}}" class="text-red-300">삭제</a></td>
                        </tr>
                        {% endfor %}
                        </tbody>
                    </table>
...
def admin_product_add():
    ...
    # 권한 체크: 마스터 혹은 해당 카테고리 담당자인지는 폼 제출 시 체크
    if request.method == 'POST':
        category_name = request.form['category']
        if not check_admin_permission(category_name):
            flash("해당 카테고리에 대한 등록 권한이 없습니다."); return redirect('/admin')
        
        # 카테고리 설정에 따른 자동 세금 할당
        cat_info = Category.query.filter_by(name=category_name).first()
        tax_type = cat_info.tax_type if cat_info else '과세'
        
        dl = datetime.strptime(request.form['deadline'], '%Y-%m-%dT%H:%M') if request.form.get('deadline') else None
        db.session.add(Product(name=request.form['name'], category=category_name, price=int(request.form['price']), spec=request.form['spec'], origin=request.form['origin'], farmer="바구니삼촌", stock=int(request.form['stock']), deadline=dl, badge=request.form['badge'], tax_type=tax_type, image_url=save_uploaded_file(request.files.get('main_image')) or "", detail_image_url=save_uploaded_file(request.files.get('detail_image')) or ""))
        db.session.commit(); return redirect('/admin')
    return render_template_string(HEADER_HTML + """<div class="max-w-xl mx-auto py-10 px-4"><h2 class="text-xl font-black mb-8 text-orange-600">새 상품 등록</h2><form method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-3xl shadow-lg space-y-5 text-xs font-bold">
        <div><label>상품 배치 카테고리</label><select name="category" class="w-full border p-4 rounded-xl">{% for c in cats %}{% if current_user.is_admin or c.manager_email == current_user.email %}<option value="{{c.name}}">{{c.name}} ({{c.tax_type}})</option>{% endif %}{% endfor %}</select></div>
        <input name="name" placeholder="상품명" class="w-full border p-4 rounded-xl" required>
        <div class="grid grid-cols-2 gap-4"><input name="price" type="number" placeholder="가격" class="w-full border p-4 rounded-xl" required><input name="spec" placeholder="규격 (예: 1kg)" class="w-full border p-4 rounded-xl"></div>
        <div class="grid grid-cols-2 gap-4"><input name="stock" type="number" placeholder="재고 (필수)" class="w-full border p-4 rounded-xl" value="50" required><input name="deadline" type="datetime-local" class="w-full border p-4 rounded-xl"></div>
        <input name="origin" placeholder="산지" class="w-full border p-4 rounded-xl" value="국산">
        <select name="badge" class="w-full border p-4 rounded-xl"><option value="">뱃지 없음</option><option value="오늘마감">🔥 오늘마감</option><option value="삼촌추천">⭐ 삼촌추천</option><option value="강력추천">💎 강력추천</option><option value="최저가">📉 최저가</option></select>
        <div><label>목록 사진 (이미지 잘림 자동방지)</label><input type="file" name="main_image" class="text-[9px]"></div>
        <div><label>상세 설명 사진</label><input type="file" name="detail_image" class="text-[9px]"></div>
        <button class="w-full bg-green-600 text-white py-5 rounded-xl font-black text-base shadow-lg">대행 상품 등록</button></form></div>""", cats=cats)
...
def admin_product_edit(pid):
    ...
    if not check_admin_permission(p.category):
        flash("이 상품을 수정할 권한이 없습니다."); return redirect('/admin')
    ...
    if request.method == 'POST':
        ...
        # 담당 카테고리 세금 타입 자동 동기화
        cat_info = Category.query.filter_by(name=p.category).first()
        if cat_info: p.tax_type = cat_info.tax_type
        ...
        db.session.commit(); return redirect('/admin')
    return render_template_string(HEADER_HTML + """<div class="max-w-xl mx-auto py-10 px-4"><h2 class="text-xl font-black mb-8 text-blue-600">상품 정보 수정</h2><form method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-3xl shadow-lg space-y-5 text-xs font-bold">
        <div><label>카테고리 이동</label><select name="category" class="w-full border p-4 rounded-xl">{% for c in cats %}{% if current_user.is_admin or c.manager_email == current_user.email %}<option value="{{c.name}}" {% if p.category == c.name %}selected{% endif %}>{{c.name}}</option>{% endif %}{% endfor %}</select></div>
        <input name="name" value="{{p.name}}" class="w-full border p-4 rounded-xl" required>
        <div class="grid grid-cols-2 gap-4"><input name="price" type="number" value="{{p.price}}" class="w-full border p-4 rounded-xl" required><input name="spec" value="{{p.spec}}" class="w-full border p-4 rounded-xl"></div>
        <div class="grid grid-cols-2 gap-4"><input name="stock" type="number" value="{{p.stock}}" class="w-full border p-4 rounded-xl" required><input name="deadline" type="datetime-local" value="{{ p.deadline.strftime('%Y-%m-%dT%H:%M') if p.deadline else '' }}" class="w-full border p-4 rounded-xl"></div>
        <input name="origin" value="{{p.origin}}" class="w-full border p-4 rounded-xl">
        <select name="badge" class="w-full border p-4 rounded-xl"><option value="" {% if not p.badge %}selected{% endif %}>없음</option><option value="오늘마감" {% if p.badge=='오늘마감' %}selected{% endif %}>🔥 오늘마감</option><option value="삼촌추천" {% if p.badge=='삼촌추천' %}selected{% endif %}>⭐ 삼촌추천</option><option value="강력추천" {% if p.badge=='강력추천' %}selected{% endif %}>💎 강력추천</option></select>
        <div><label class="text-green-600">목록 사진 변경 시에만 선택</label><input type="file" name="main_image" class="text-[9px]"></div>
        <button class="w-full bg-blue-600 text-white py-5 rounded-xl font-black text-base shadow-lg">수정 완료</button></form></div>""", p=p, cats=cats)

@app.route('/admin/delete/<int:pid>')
@login_required
def admin_delete(pid):
    p = Product.query.get(pid)
    if p and check_admin_permission(p.category):
...
def admin_category_add():
    ...
    name, tax, manager = request.form['cat_name'], request.form['tax_type'], request.form.get('manager_email', '').strip()
    db.session.add(Category(name=name, tax_type=tax, manager_email=manager if manager else None))
...
def admin_orders_excel():
    # 카테고리 담당자는 엑셀 다운로드 시에도 본인 관련 상품이 포함된 주문만 보게 하거나 보안상 마스터만 가능하게 설정 가능
    if not current_user.is_admin: return redirect('/admin')
    data = [{"일시": o.created_at.strftime('%Y-%m-%d %H:%M'), "고객": o.customer_name, "전화": o.customer_phone, "주소": o.delivery_address, "상품": o.product_details, "총액": o.total_price, "면세액": o.tax_free_amount} for o in Order.query.all()]
...
def init_db():
    with app.app_context():
        ...
        if not Category.query.first():
            db.session.add(Category(name="농산물", tax_type="면세", manager_email=None))
            db.session.add(Category(name="공동구매", tax_type="과세", manager_email=None))
            db.session.add(Category(name="반찬", tax_type="과세", manager_email=None))
        
        # 테스트 상품
        if not Product.query.first():
            items = [
                ("농산물", "산지직송 꿀부사 사과", 12000, "2kg", "청송", "삼촌추천", "면세", 20),
                ("농산물", "제주 당도타이벡 감귤", 8500, "3kg", "제주", "오늘마감", "면세", 15),
                ("공동구매", "대용량 베이킹소다 세제", 15900, "4L x 2", "국산", "강력추천", "과세", 50),
                ("반찬", "고소한 견과류 멸치볶음", 6500, "150g", "국산", "삼촌추천", "과세", 10)
            ]
