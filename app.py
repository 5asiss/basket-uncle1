...
def index():
    ...
    if view == 'home':
        ...
        <div class="grid grid-cols-2 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            <a href="/?view=ready" class="bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition border-4 border-orange-50 flex flex-col items-center group">
                ...
                <span class="text-xs text-gray-400 mt-2 text-center italic">산지 직송 신선함 그대로</span>
            </a>
            <a href="/?view=mart" class="bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition border-4 border-green-50 flex flex-col items-center group">
                ...
                <span class="text-xs text-gray-400 mt-2 text-center italic">식자재마트 가격 그대로</span>
            </a>
            <a href="/?view=ready" class="bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition border-4 border-red-50 flex flex-col items-center group">
                ...
                <span class="text-xs text-gray-400 mt-2 text-center italic">생활용품 대리구매</span>
            </a>
            <a href="/?view=ready" class="bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition border-4 border-blue-50 flex flex-col items-center group">
                ...
                <span class="text-xs text-gray-400 mt-2 text-center italic">동네 맛집 삼촌 배달</span>
            </a>
        </div>
        """
    ...
    # --- 🌟 마트 장보기 화면 (상세 쇼핑몰) ---
        ...
        <div class="mb-4 flex items-center gap-2 text-left">
            <a href="/" class="bg-white p-2 rounded-full shadow-sm text-gray-400 hover:text-green-600 transition">← 홈으로</a>
        ...
        content = """<div class="py-32 text-center"><span class="text-8xl mb-8 block">👷‍♂️</span><h2 class="text-3xl font-black text-gray-800">서비스 준비 중입니다</h2><p class="text-gray-400 mt-4 font-medium leading-relaxed text-sm">삼촌이 더 신선하고 다양한 서비스를 <br>제공하기 위해 열심히 준비하고 있어요!</p><a href="/" class="inline-block mt-10 bg-green-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg hover:bg-green-700 transition">홈으로 돌아가기</a></div>"""
        return render_template_string(HEADER_HTML + content + FOOTER_HTML)
...
def cart():
    ...
    content = """<h2 class="text-2xl font-black mb-8 text-left text-gray-800">장바구니 확인</h2><div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 text-left">{% if items %}<div class="p-8 space-y-6">{% for i in items %}<div class="flex justify-between items-center bg-gray-50 p-4 rounded-2xl text-sm"><div class="flex-1 text-left"><p class="font-black text-gray-800 leading-tight">{{ i.product_name }}</p><p class="text-green-600 font-bold mt-1 text-xs">{{ i.price }}원 x {{ i.quantity }}개</p></div><span class="font-black text-gray-900 text-lg">{{ i.price * i.quantity }}원</span><a href="/cart/delete/{{ i.id }}" class="text-[10px] text-red-400 ml-4 font-bold hover:underline">삭제</a></div>{% endfor %}</div><div class="bg-green-600 p-8 flex justify-between items-center text-white text-left"><div><span class="text-green-200 text-xs">최종 합계 금액</span><p class="text-3xl font-black">{{ total }}원</p></div><a href="/order/confirm" class="bg-white text-green-700 px-10 py-4 rounded-2xl font-black text-lg shadow-lg hover:bg-green-50 transition">주문하기</a></div>{% else %}<div class="py-20 text-center font-bold text-gray-400 text-sm">장바구니가 텅 비어있어요.</div>{% endif %}</div>"""
...
@login_required
def order_confirm():
    content = """<div class="max-w-2xl mx-auto bg-white p-10 rounded-[2.5rem] shadow-2xl mt-6 border-4 border-green-50 text-sm text-left"><h2 class="text-2xl font-black mb-8 text-green-800 text-center underline decoration-green-100 underline-offset-8">주문 정보 최종 확인</h2><div class="mb-10 space-y-4"><h4 class="font-bold text-gray-400 border-b pb-2 text-sm">배송지 및 출입정보</h4><p class="text-lg font-black text-gray-800">{{ current_user.name }}님 / {{ current_user.phone }}</p><p class="bg-yellow-50 p-4 rounded-xl font-bold text-gray-700 leading-relaxed border border-yellow-100 shadow-sm">🏠 {{ current_user.address }}<br>{{ current_user.address_detail }}</p><p class="bg-red-50 p-4 rounded-xl text-red-800 font-black text-xs border border-red-100 shadow-sm">🔑 공동현관: {{ current_user.entrance_pw }}</p></div><form action="/order/submit" method="POST"><button class="w-full bg-green-600 text-white py-5 rounded-3xl font-black text-xl hover:bg-green-700 shadow-xl transition transform active:scale-95">주문 최종 전송</button></form></div>"""
    return render_template_string(HEADER_HTML + content + FOOTER_HTML)
...
def mypage():
    ...
    content = """<h2 class="text-2xl font-black mb-8 text-left text-gray-800">내 주문 내역</h2><div class="space-y-6 text-left text-sm">{% for o in orders %}<div class="bg-white p-8 rounded-3xl shadow-sm border text-sm hover:shadow-md transition"><div><span class="text-[10px] text-gray-400 font-bold italic">{{ o.created_at.strftime('%Y-%m-%d %H:%M') }}</span><p class="font-black text-gray-800 mt-1 text-base leading-tight">{{ o.product_details }}</p><p class="text-gray-500 text-[10px] mt-2 italic">배송지: {{ o.address }} {{ o.address_detail }}</p></div><div class="flex justify-between items-center mt-4 border-t pt-4"><span class="text-xl font-black text-green-600">{{ o.total_price }}원</span><span class="bg-green-50 text-green-700 px-3 py-1 rounded-full text-[10px] font-bold shadow-sm">{{ o.status }}</span></div></div>{% endfor %}</div>"""
...
def admin_products():
    ...
    <div class="flex flex-col gap-6 mb-6 text-left">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm"><h2 class="text-xl font-bold">상품 통합 관리</h2><div class="flex gap-2"><a href="/admin/users" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl text-xs font-bold shadow-sm hover:bg-gray-200">회원 관리</a><a href="/admin/add" class="bg-green-600 text-white px-4 py-2 rounded-xl font-bold text-xs shadow-sm hover:bg-green-700">+ 새 상품 등록</a></div></div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border text-xs">
            ...
            <form action="/admin/upload" method="POST" enctype="multipart/form-data" class="flex gap-4">
                <input type="file" name="excel_file" accept=".xlsx, .xls" class="border-2 border-dashed p-3 rounded-xl w-full cursor-pointer hover:border-blue-400" required>
                <button type="submit" class="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold whitespace-nowrap shadow-md hover:bg-blue-700 transition">업로드 시작</button>
            </form>
            <p class="mt-2 text-gray-400 italic">양식: 카테고리번호 | 상품명 | 규격 | 가격 | 이미지파일명</p>
        </div>

        <div class="bg-white p-6 rounded-3xl shadow-sm border text-xs">
            <h4 class="font-bold mb-4">📁 카테고리별 상품 필터</h4>
            <div class="flex flex-wrap gap-2 mb-4">
                <a href="/admin/products" class="px-4 py-2 bg-gray-100 rounded-full font-bold hover:bg-gray-200 transition">전체보기</a>
                {% for c in categories %}
                <div class="flex items-center gap-1 bg-green-50 px-3 py-1.5 rounded-full shadow-sm">
                    <a href="/admin/products?category={{c.id}}" class="text-green-700 font-bold hover:underline">{{c.name}}</a>
                    <a href="/admin/category/delete_all/{{c.id}}" class="text-red-400 font-black ml-1 text-xs hover:text-red-600" onclick="return confirm('해당 카테고리 상품을 모두 삭제할까요?')">×</a>
                </div>
                {% endfor %}
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border overflow-hidden">
            <table class="w-full text-left text-[11px]">
                <thead class="bg-gray-50 border-b"><tr><th class="p-5">상품명/규격</th><th class="p-5 text-center">판매가</th><th class="p-5">상태</th><th class="p-5 text-center">관리</th></tr></thead>
                <tbody>{% for p in products %}<tr class="border-b hover:bg-gray-50 transition">
                    ...
                    <td class="p-5 text-center font-bold text-gray-800">{{ p.price_retail }}원</td>
...
def admin_users():
    ...
    <div class="flex flex-col gap-6 text-left">
        <div class="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm"><h2 class="text-xl font-bold text-gray-800 text-sm">회원 통합 관리</h2><a href="/admin/users/excel" class="bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md hover:bg-blue-700 transition">회원 엑셀 다운로드</a></div>
        <div class="bg-white rounded-3xl shadow-sm border overflow-hidden">
            <table class="w-full text-left text-[10px]">
                <thead class="bg-gray-50 border-b"><tr><th class="p-5">이름/아이디/연락처</th><th class="p-5 text-center">배송지 상세</th><th class="p-5 text-center">출입비번</th></tr></thead>
                <tbody>{% for u in users %}<tr class="border-b hover:bg-gray-50 transition"><td class="p-5"><b>{{ u.name }}</b><br><span class="text-gray-400">{{ u.email }}</span><br>{{ u.phone }}</td><td class="p-5 leading-relaxed">{{ u.address }}<br><span class="text-green-600 font-bold">{{ u.address_detail }}</span></td><td class="p-5 text-center font-bold text-gray-800">🔑 {{ u.entrance_pw }}</td></tr>{% endfor %}</tbody>
            </table>
        </div>
    </div>
    """
...
def admin_upload():
    ...
    if file:
        try:
            ...
            db.session.commit(); flash("엑셀 업로드 성공!")
